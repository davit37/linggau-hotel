...
function list_hotel(){
	$query = query("SELECT id_hotel,nama_hotel from hotel ");
	while ($row = mysql_fetch_row($query)) {
	
			echo "<li><a href='index.php?mod=page&pg=home&id_hotel=".$row[0]."'>" . ucwords($row[1]) . "</a> </li>";
...
function list_news($jumlah) {
	...
	$query = query("SELECT idberita,`hotel`.`nama_hotel`,judul FROM hotel JOIN berita ON `hotel`.`id_hotel`=`berita`.`id_hotel` order by tanggal desc limit $jumlah");
	if(isset($_GET['id_hotel'])){
		$query = query("SELECT idberita,`hotel`.`nama_hotel`,judul FROM hotel JOIN berita ON `hotel`.`id_hotel`=`berita`.`id_hotel` WHERE `hotel`.`id_hotel`='$_GET[id_hotel]'order by tanggal desc limit $jumlah");
	}
	while ($row = mysql_fetch_row($query)) {
	
			echo "<li><a href='index.php?mod=page&pg=berita&idberita=".$row[0]."'>" . ucwords($row[1])." - ".ucwords($row[2]) . "</a> </li>";
...
function get_status_stok($jumlah) {
	  ...
	  if ($jumlah == '0') {
		return 'sisa kamar habis';
	} else  {
		return $jumlah. " Kamar Tersedia";
	}
}
/* fungsi bantu style*/
...
function combo_hotel($kode) {
	echo "<option value='' selected>- Pilih hotel -</option>";
