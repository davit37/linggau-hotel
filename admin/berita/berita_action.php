...
if($aksi == 'tambah') {
	$sql = "INSERT INTO berita(id_hotel,judul,gambar,tanggal,isi,aktif)
		VALUES('$_POST[id_hotel]','$judul','$gambar_file',now(),'$isi','1')";
}else if($aksi== 'edit') {
	if($ukuran_file > 0){
	$sql = "update berita set judul='$judul',
	gambar='$gambar_file',isi='$isi' ,id_hotel='$_POST[id_hotel]'
	where idberita='$id'";
	}else{
		$sql = "update berita set judul='$judul',
	isi='$isi'
